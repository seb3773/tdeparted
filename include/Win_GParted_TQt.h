...
class ImagingWorkerBase_TQt
{
public:
	virtual ~ImagingWorkerBase_TQt() = default;
	virtual void start() = 0;
	virtual bool running() = 0;
	virtual void wait() = 0;
	virtual void cancel() = 0;
	virtual bool get_success() = 0;
	virtual bool get_cancelled() = 0;
	virtual void get_updates( std::vector<ImagingUpdate_TQt> &updates ) = 0;
};
...
private slots:
	...
	void menu_device_convert_partition_table();
	...
	void action_partition_wipe();
	void action_partition_mount_options();
...
protected:
	...
	int m_menubar_edit_id;
	int m_menubar_view_id;
	int m_menubar_device_id;
	int m_menubar_partition_id;
	...
	int m_menu_device_convert_table_id;
	...
	int m_menu_partition_wipe_id;
	int m_menu_partition_mount_options_id;
...
class Dialog_Convert_Partition_Table_TQt : public TQDialog
{
	TQ_OBJECT

public:
	Dialog_Convert_Partition_Table_TQt( const Device &device, TQWidget *parent );
	Glib::ustring target_disklabel() const;

private slots:
	void on_target_changed( int index );

private:
	const Device &m_device;
	TQComboBox *m_combo;
	TQLabel *m_lbl_current;
	TQLabel *m_lbl_warn;
	TQPushButton *m_btn_apply;
	TQString m_warn_gpt;
	TQString m_warn_msdos;
	bool m_ok_gpt;
	bool m_ok_msdos;
};
...
class ClipboardCopy_TQt : public TQObject
{
	TQ_OBJECT

public:
	ClipboardCopy_TQt( TQObject *parent );
	void set_text( const TQString &text );
...
protected:
	...
	std::map<Glib::ustring, int> m_last_status;
...
class ImagingWorker_TQt : public TQThread, public ImagingWorkerBase_TQt
{
public:
	virtual ~ImagingWorker_TQt() = default;
...
public:
	...
	bool run_modal_imaging( ImagingWorkerBase_TQt *worker );
...
private:
	...
	ImagingWorkerBase_TQt *m_worker;
	TQTimer *m_ui_timer;
	TQPixmap m_icon_execute;
	TQPixmap m_icon_success;
	TQPixmap m_icon_error;
...
class Dialog_Fstab_Options_TQt : public TQDialog
{
	TQ_OBJECT

public:
	Dialog_Fstab_Options_TQt( const Partition &partition, TQWidget *parent );
...
private:
	...
	void load_from_fstab();
	bool validate_fields( TQString &error ) const;
	TQString build_fstab_line() const;
	bool save_fstab_update( const TQString &new_line, TQString &error );
	static TQString help_fields_text();
	static TQString help_syntax_text_for_type( const TQString &fs_type );
	...
	TQSpinBox *m_spin_pass;
	TQTextEdit *m_preview;
	TQPushButton *m_btn_fields_help;
	TQPushButton *m_btn_syntax_help;
	TQPushButton *m_btn_save;
	TQPushButton *m_btn_cancel;
	TQString m_fstab_fsname;
};
