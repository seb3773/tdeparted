...
# ==============================================================================
# tdeParted - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " tdeParted - Q4OS .qsi Package Builder"
echo "=================================================="
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/tdeparted_[0-9]*_*.deb "$SCRIPT_DIR"/*.deb "$SCRIPT_DIR"/build/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "tdeparted")
...
APPNAME_DESC="tdeParted Partition Editor"
APP_ICON="tdeparted"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
