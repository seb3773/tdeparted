                            ...
                            const Glib::ustring & link_id  /* For context sensitive help */)
{
	...
	Gtk::MessageDialog dialog( *this,
					...
					Gtk::BUTTONS_OK,
					true ) ;
	Glib::ustring tmp_msg = _( "Help is not available in this build." ) ;
	dialog .set_secondary_text( tmp_msg ) ;
	dialog .run() ;
}
...
void Win_GParted::menu_help_about()
{
	...
	dialog.set_program_name(_("tdeParted"));
	...
	dialog.set_website_label( "tdeParted" );
...
void Win_GParted::activate_resize()
{
	    ...
	    ask_for_password_for_encrypted_resize_as_required(*selected_partition_ptr)   )
	{
		     ...
		     operation->get_partition_original().type         != TYPE_EXTENDED                                  )
		{
			...
			tmp_msg += "\n";
			tmp_msg += _("You can learn how to repair the boot configuration in the documentation.");
			tmp_msg += "\n\n";
