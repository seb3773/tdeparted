...
static PedExceptionOption tqt_ped_exception_dialog( PedException *e )
{
	...
	dlg.setCaption(TQString::fromUtf8(caption));
	...
	PedExceptionOption selected = PED_EXCEPTION_UNHANDLED;
	for (unsigned int i = 0; i < sizeof(buttons)/sizeof(buttons[0]); i++)
	{
		...
		TQObject::connect(btn, TQT_SIGNAL(clicked()), &dlg, TQT_SLOT(accept()));
		if (selected == PED_EXCEPTION_UNHANDLED)
			selected = buttons[i].opt;
	}
