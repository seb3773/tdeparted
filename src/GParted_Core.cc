...
static Glib::RefPtr<Glib::MainLoop> gparted_foreground_loop;

static const Glib::ustring GPARTED_BUG( _( "tdeParted Bug" ) );
...
{
	Glib::ustring str = Glib::ustring("tdeParted ") + VERSION + "\n";
...
void GParted_Core::set_devices( std::vector<Device> & devices )
{
	Glib::Thread::create( sigc::bind(
				...
				&devices),
			      false );
	gparted_foreground_loop = Glib::MainLoop::create(Glib::MainContext::get_default(), false);
	gparted_foreground_loop->run();
	gparted_foreground_loop.reset();
}

static bool _mainquit( void *dummy )
{
	if (gparted_foreground_loop)
		gparted_foreground_loop->quit();
	return false;
}
...
bool GParted_Core::apply_operation_to_disk( Operation * operation )
{
	...
	switch (operation->m_type)
	{
		...
		{
			OperationCreatePartitionTable *op = dynamic_cast<OperationCreatePartitionTable *>( operation );
			if (!op)
			{
				success = false;
				break;
			}
			operation->m_operation_detail.add_child( OperationDetail( Glib::ustring::compose( _( "create partition table %1" ), op->get_new_disklabel() ) ) );
			OperationDetail &child = operation->m_operation_detail.get_last_child();
			success = set_disklabel( op->m_device, op->get_new_disklabel() );
			child.set_success_and_capture_errors( success );
			break;
		}

		case OPERATION_CONVERT_PARTITION_TABLE:
		{
			OperationConvertPartitionTable *op = dynamic_cast<OperationConvertPartitionTable *>( operation );
			if (!op)
			{
				success = false;
				break;
			}
			const Glib::ustring &dev = op->m_device.get_path();
			operation->m_operation_detail.add_child( OperationDetail( Glib::ustring::compose( _( "convert partition table on %1" ), dev ) ) );
			OperationDetail &root = operation->m_operation_detail.get_last_child();

			if (Glib::find_program_in_path( "sgdisk" ).empty())
			{
				root.add_child( OperationDetail( _( "Missing required tool: sgdisk" ), STATUS_ERROR, FONT_ITALIC ) );
				root.set_success_and_capture_errors( false );
				success = false;
				break;
			}
			if (Glib::find_program_in_path( "partprobe" ).empty())
			{
				root.add_child( OperationDetail( _( "Missing required tool: partprobe" ), STATUS_ERROR, FONT_ITALIC ) );
				root.set_success_and_capture_errors( false );
				success = false;
				break;
			}
			...
			Glib::ustring backup = "/var/tmp/tdeparted_backup_";
			backup += ts;
			backup += ".gpt";
			...
			root.add_child( OperationDetail( Glib::ustring::compose( _( "backup partition table to %1" ), backup ) ) );
			st = Utils::execute_command( Glib::ustring( "sgdisk -b " ) + Glib::shell_quote( backup ) + " " + Glib::shell_quote( dev ), out, err, true );
			root.get_last_child().set_success_and_capture_errors( st == 0 );
			if (st != 0)
			{
				root.set_success_and_capture_errors( false );
				success = false;
			...
			out.clear();
			err.clear();
			if (op->get_from_disklabel() == "msdos" && op->get_to_disklabel() == "gpt")
			{
				root.add_child( OperationDetail( _( "convert MBR to GPT" ) ) );
				st = Utils::execute_command( Glib::ustring( "sgdisk -g " ) + Glib::shell_quote( dev ), out, err, true );
				root.get_last_child().set_success_and_capture_errors( st == 0 );
			}
			else if (op->get_from_disklabel() == "gpt" && op->get_to_disklabel() == "msdos")
			{
				Glib::ustring part_list;
				unsigned int count = 0;
				PedDevice *lp_device_list = nullptr;
				PedDisk *lp_disk_list = nullptr;
				if (!get_device( dev, lp_device_list, true ) || !get_disk( lp_device_list, lp_disk_list ))
				{
					destroy_device_and_disk( lp_device_list, lp_disk_list );
					root.add_child( OperationDetail( _( "Failed to read current partition table" ), STATUS_ERROR, FONT_ITALIC ) );
					root.get_last_child().set_success_and_capture_errors( false );
					root.set_success_and_capture_errors( false );
					success = false;
					break;
				}
				for (PedPartition *pp = ped_disk_next_partition( lp_disk_list, nullptr );
				     pp;
				     pp = ped_disk_next_partition( lp_disk_list, pp ))
				{
					if (pp->num <= 0)
						continue;
					count++;
					...
					if (!part_list.empty())
						part_list += ":";
					part_list += Utils::num_to_str( pp->num );
				}
				destroy_device_and_disk( lp_device_list, lp_disk_list );
				if (count == 0)
				{
					root.add_child( OperationDetail( _( "No partitions found to convert to MBR" ), STATUS_ERROR, FONT_ITALIC ) );
					root.get_last_child().set_success_and_capture_errors( false );
					root.set_success_and_capture_errors( false );
					success = false;
					break;
				...
				if (count > 4)
				{
					root.add_child( OperationDetail( _( "GPT to MBR conversion supports a maximum of 4 partitions" ), STATUS_ERROR, FONT_ITALIC ) );
					root.get_last_child().set_success_and_capture_errors( false );
					root.set_success_and_capture_errors( false );
					success = false;
					break;
				}
				root.add_child( OperationDetail( _( "convert GPT to MBR" ) ) );
				st = Utils::execute_command( Glib::ustring( "sgdisk -m " ) + part_list + " " + Glib::shell_quote( dev ), out, err, true );
				root.get_last_child().set_success_and_capture_errors( st == 0 );
			}
			else
			{
				root.add_child( OperationDetail( _( "Unsupported conversion" ), STATUS_ERROR, FONT_ITALIC ) );
				root.get_last_child().set_success_and_capture_errors( false );
				root.set_success_and_capture_errors( false );
				success = false;
				break;
			...
			out.clear();
			err.clear();
			root.add_child( OperationDetail( _( "inform the kernel of partition table changes" ) ) );
			st = Utils::execute_command( Glib::ustring( "partprobe " ) + Glib::shell_quote( dev ), out, err, true );
			root.get_last_child().set_success_and_capture_errors( st == 0 );

			// Ensure udev settled after the conversion so subsequent scans are stable.
			settle_device( SETTLE_DEVICE_APPLY_MAX_WAIT_SECONDS );

			success = (st == 0);
			if (success)
			{
				PedDevice *lp_device_verify = nullptr;
				PedDisk *lp_disk_verify = nullptr;
				Glib::ustring observed;
				if (get_device( dev, lp_device_verify, true ) && get_disk( lp_device_verify, lp_disk_verify ) &&
				    lp_disk_verify && lp_disk_verify->type && lp_disk_verify->type->name)
					observed = lp_disk_verify->type->name;
				destroy_device_and_disk( lp_device_verify, lp_disk_verify );
				if (!observed.empty() && observed != op->get_to_disklabel())
				{
					root.add_child( OperationDetail( Glib::ustring::compose( _( "Partition table verification failed (still %1)" ), observed ), STATUS_ERROR, FONT_ITALIC ) );
					root.get_last_child().set_success_and_capture_errors( false );
					success = false;
				}
			}
			root.set_success_and_capture_errors( success );
			break;
		}
	}
                                       ...
                                       std::vector<Glib::ustring> &messages)
{
	...
	temp += "\n- " ; 
	temp += _( "The file system is unknown to tdeParted" ) ;
	temp += "\n- "; 
				    	   ...
				    	   OperationDetail & operationdetail ) 
{
	...
	auto lp_resize_on_main = []( lp_resize_ctx *ctx ) -> gboolean {
		const bool ok = ped_file_system_resize( ctx->fs, ctx->geom, nullptr );
		ctx->mutex.lock();
		ctx->ret = ok;
		ctx->done = true;
		ctx->cond.signal();
		...
		if ( lp_geom )
		{
			...
			if ( fs )
			{
				...
				if ( lp_geom )
				{
					...
					ctx.done = false;
					ctx.mutex.lock();
					g_idle_add( (GSourceFunc)+lp_resize_on_main, &ctx );
					while ( ! ctx.done )
						ctx.cond.wait( ctx.mutex );
					ctx.mutex.unlock();
					return_value = ctx.ret;
...
static PedExceptionOption tqt_ped_exception_dialog( PedException *e )
{
	if (e == nullptr)
	...
	switch (e->type)
	{
		case PED_EXCEPTION_INFORMATION: caption = _("Libparted Information"); break;
		case PED_EXCEPTION_WARNING:     caption = _("Libparted Warning"); break;
		case PED_EXCEPTION_ERROR:       caption = _("Libparted Error"); break;
		case PED_EXCEPTION_FATAL:       caption = _("Libparted Fatal"); break;
		case PED_EXCEPTION_BUG:         caption = _("Libparted Bug"); break;
		case PED_EXCEPTION_NO_FEATURE:  caption = _("Libparted Unsupported Feature"); break;
		default:                        caption = _("Libparted unknown exception"); break;
	}

	const TQString msg = TQString::fromUtf8(e->message ? e->message : "");
	const TQString cap = TQString::fromUtf8(caption);

	TQDialog dlg(nullptr, "ped_exception", true);
	...
	label->setAlignment(TQt::WordBreak | TQt::AlignLeft | TQt::AlignTop);
	label->setSizePolicy(TQSizePolicy::Preferred, TQSizePolicy::Preferred);
	label->setMaximumWidth(700);

	dlg.setCaption(cap);
	dlg.resize(700, dlg.sizeHint().height());

	TQHBox *hbox = new TQHBox(vbox);
	hbox->setSpacing(8);
	TQSignalMapper *mapper = new TQSignalMapper(&dlg);
	TQObject::connect(mapper, TQT_SIGNAL(mapped(int)), &dlg, TQT_SLOT(done(int)));
	...
	static const ButtonMap buttons[] = {
		{ PED_EXCEPTION_FIX,    _("Fix") },
		{ PED_EXCEPTION_YES,    _("Yes") },
		{ PED_EXCEPTION_OK,     _("Ok") },
		{ PED_EXCEPTION_RETRY,  _("Retry") },
		{ PED_EXCEPTION_NO,     _("No") },
		{ PED_EXCEPTION_CANCEL, _("Cancel") },
		{ PED_EXCEPTION_IGNORE, _("Ignore") }
	};

	for (unsigned int i = 0; i < sizeof(buttons)/sizeof(buttons[0]); i++)
	{
		if (!(e->options & buttons[i].opt))
			continue;
		TQPushButton *btn = new TQPushButton(TQString::fromUtf8(buttons[i].text), hbox);
		btn->setMinimumWidth(90);
		mapper->setMapping(btn, (int)buttons[i].opt);
		TQObject::connect(btn, TQT_SIGNAL(clicked()), mapper, TQT_SLOT(map()));
	}

	const int r = dlg.exec();
	if (r <= 0)
		return PED_EXCEPTION_UNHANDLED;
	return (PedExceptionOption)r;
}
...
static bool _ped_exception_handler( struct ped_exception_ctx *ctx )
{
	...
	ctx->ret = tqt_ped_exception_dialog( ctx->e );
	ctx->mutex.lock();
	ctx->cond.signal();
	ctx->mutex.unlock();
	return false;
