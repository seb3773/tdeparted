...
static Glib::RefPtr<Glib::MainLoop> command_foreground_loop;
...
static void execute_command_eof()
{
	...
	{
		if (command_foreground_loop)
			command_foreground_loop->quit();
	}
}
...
void OperationDetail::add_child( const OperationDetail & operationdetail )
{
	if ( no_more_children )
		...
		// added anyway.
		add_child_implement( OperationDetail( Glib::ustring( _("tdeParted Bug") ) + ": " +
		                                      /* TO TRANSLATORS:
                                              ...
                                              TimedSlot timed_progress_slot)
{
	...
	command_foreground_loop = Glib::MainLoop::create(Glib::MainContext::get_default(), false);
	command_foreground_loop->run();
	command_foreground_loop.reset();
