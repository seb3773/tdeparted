...
{
	// Only add node path if mount point exists, or for swap/no-mountpoint entries
	if ( mountpoint.empty() || Glib::file_test(mountpoint, Glib::FILE_TEST_EXISTS) )
	{
...
const MountEntry& Mount_Info::find(MountMapping& map, const Glib::ustring& path)
{
	...
	for (iter_mp = map.begin(); iter_mp != map.end(); ++iter_mp)
	{
		if (iter_mp->first.m_name.compare(0, 5, "UUID=")  == 0 ||
		    ...
		    iter_mp->first.m_name.compare(0, 9, "PARTUUID=") == 0   )
		{
...
Glib::ustring Mount_Info::lookup_uuid_or_label(const Glib::ustring& ref)
{
	...
	if (ref.compare(0, 9, "PARTUUID=") == 0)
		return FS_Info::get_path_by_partuuid(ref.substr(9));
