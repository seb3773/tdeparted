...
// Retrieve the path given the PARTUUID
Glib::ustring FS_Info::get_path_by_partuuid( const Glib::ustring & partuuid )
{
	if (not_initialised_then_error())
		return "";

	for ( unsigned int i = 0 ; i < fs_info_cache.size() ; i ++ )
		if ( partuuid == fs_info_cache[i].partuuid )
...
// Retrieve the PARTUUID given for the path
Glib::ustring FS_Info::get_partuuid( const Glib::ustring & path )
{
	if (not_initialised_then_error())
		return "";

	const FS_Entry & fs_entry = get_cache_entry_by_path( path );
	return fs_entry.partuuid;
}
...
bool FS_Info::not_initialised_then_error()
{
	if (! fs_info_cache_initialized)
		std::cerr << "tdeParted Bug: FS_Info (blkid) cache not loaded before use" << std::endl;
...
void FS_Info::run_blkid_load_cache(const std::vector<Glib::ustring>& paths)
{
	...
	for (unsigned int i = 0; i < lines.size(); i++)
	{
		...
		if (entry_path.length() > 0)
		{
			...
			fs_entry.partuuid = Utils::regexp_label(lines[i], " PARTUUID=\"([^\"]*)\"");
